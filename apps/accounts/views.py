...
class GlobalSearchView(LoginRequiredMixin, View):
    ...
    Delegates to GlobalSearchService for comprehensive RBAC, module, and operational search.
    """
    def get(self, request):
        ...
        try:
            ...
            import logging
            logger = logging.getLogger(__name__)
            logger.exception("Global search failed for user %s with query '%s': %s", request.user, query, e)
            results = []
            error_message = "An error occurred while performing search. Please try again."

        return render(request, 'cotton/search_results_partial.html', {
            'results': results,
            'query': query,
