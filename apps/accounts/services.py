...
class RoleAssignmentService:
    ...
    - Preserves protected roles (system_owner, super_admin)
    - Prevents privilege escalation: non-superusers cannot grant roles exceeding their effective access
    - Records assigned_by
    ...
    def get_assignable_roles_queryset(cls, actor=None):
        ...
        if actor and not actor.is_superuser and actor.is_authenticated:
    ...
    @classmethod
    def validate_role_authority(cls, actor, target_roles: Iterable[Role]):
        """
        Validates whether actor has authority to grant each role in target_roles.
        Raises PermissionDenied if actor lacks authority.
        """
        if not actor:
            return
    ...
    def sync_user_roles(
        ...
        *,
        ...
        - Calculates added_roles and removed_roles
        - Retains protected roles (e.g. system_owner, super_admin when actor is not superuser)
        ...
        cls.validate_role_authority(actor, target_roles_list)
        ...
        # 7. Update compatibility persona on CustomUser
